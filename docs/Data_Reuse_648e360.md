...
-   The conditions for a return value context as described for the `execute` method of `sap.ui.model.odata.v4.ODataContextBinding` are fulfilled.
    ...
    For more information, see the [API Reference: `sap.ui.model.odata.v4.ODataContextBinding`](https://openui5.hana.ondemand.com/#/api/sap.ui.model.odata.v4.ODataContextBinding%23methods/execute). 
...
For use cases like value help controls, the following conditions may apply:
...
In this case, the same data needs to be requested only once.

For this, you may use the binding parameter `$$sharedRequest` for all the list bindings that do not need to request the data individually.
...
**Example:** The binding parameter `$$sharedRequest`:
...
The binding parameter `$$sharedRequest` is used automatically for list bindings of [value list](Value_Lists_ab267a6.md) models.
