...
> 
> You can view and download the files for all steps in the Demo Kit at [Testing Apps](https://openui5.hana.ondemand.com/explored.html#/entity/sap.m.tutorial.testing/samples). Copy the code to your workspace and make sure that the application runs by calling the `webapp/test/test.html` file. Depending on your development environment you might have to adjust resource paths and configuration entries. You can do this in SAP Web IDE by adding a `neo-app.json` file \(from a template project\) and changing the source path from "`../../../../../../../../../../resources/sap/ui/...`" to "`../../resources/sap/ui/...`" in both `unitTests.qunit.html` and `opaTest.qunit.html` files.
> 
