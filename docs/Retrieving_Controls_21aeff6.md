...
As of version 1.72, OPA5 supports the declarative matcher syntax which allows you to declare built-in matchers in a literal object.
The syntax is inspired by control locators in [UIVeri5](https://github.com/SAP/ui5-uiveri5) and promotes reuse between the two testing tools.
A matcher declaration is a JSON object. The OPA5 `waitFor` statement is simplified by using a single JSON object, instead of the more verbose matcher instances.
Currently, only built-in matchers are allowed. Inline matcher functions and custom matcher instances are only allowed in the `matchers` `waitFor` parameter:
...
-   On the top level. In this case, if you use an unknown matcher, an exception will be thrown, stating that the parameter is not defined in OPA5 API.
...
-   In the `matchers` parameter. In this case, if you use an unknown matcher, an exception will be thrown, stating that the matcher is not supported.
...
If there are matchers declared in both places, they will be combined.

#### Matcher properties

A matcher is declared by its name and properties. The name is a key in the matchers object literal and should start with a lower-case letter. For example, to declare a `sap.ui.test.matchers.Properties` matcher, use the name `properties`.
Every matcher accepts a specific set of properties which should be declared as a value in the matchers object. This value should be an object literal.
Behind the scenes, every matcher declaration is transformed into a matcher instance. Every value in the declaration represents the properties that will
be fed to one matcher instance. There is an example for every built-in matcher in the API documentation. The following two `waitFor` statements produce the same set of matchers:
...
If you need to use one matcher twice, the value for the matcher should be an array. Each element of the array should be an object literal that will be
used by one matcher instance. This is useful when you need to match a control by two or more of its properties. The following two `waitFor` statements produce the same set of matchers:
...
When declaring a `sap.ui.test.matchers.Ancestor` or `sap.ui.test.matchers.Descendant` you need to declare the control which will be an ancestor or descendant, respectively. With matcher instances, you simply pass the control instance, that you have already located in a previous `waitFor` statement. But with matcher declarations
you cannot use object instances or functions as values. The solution is to use a nested declarative matcher for the ancestor or descendant. It is assumed that it will match exactly one control and if it doesn't, any one of the matches will be used. This matcher is special and actually supports an superset of matchers, including controlType, ID and any other JSON-compatible properties, available in a typical `waitFor` statement. The following two `waitFor` statements produce the same result:
